// Unique hits are counted using a hashed ip and user-agent.
// Visits are saved to memory immediately, and saved to disk every 1 hour.
// At the start of a new day (12am) the counting data is cleared, and stats older than 7 days are archived.
...
const visitCountArchiveFilePath = path.join(__dirname, '../data/visitCounts-archive.jsonl');
if (!fs.existsSync(visitCountFilePath)) {
    fs.writeFileSync(visitCountFilePath, JSON.stringify({}));
}
if (!fs.existsSync(visitCountArchiveFilePath)) {
    fs.writeFileSync(visitCountArchiveFilePath, '');
}

const appHitIpSetToday = new Set();
const postHitIpSetToday = new Set();
...
const UNIQUE_POST_HITS_KEY = 'uniquePostHits';
...
function visitCounterMiddleware(req, res, next) {
    try {
        ...
        if (req.method === 'GET' && !shouldSkipCounting) {
            countUniqueVisit(req.ip, req.headers['user-agent'], appHitIpSetToday);
        }
        ...
        if (req.method === 'POST' && lowerCasePath === '/track-event' && ['pageview', 'print'].includes(req.body?.event) && typeof req.body?.pathname === 'string') {
            ...
            if (isPost || path === '/') {
                ...
                if (req.body.event === 'pageview' && isPost) countUniqueVisit(req.ip, req.headers['user-agent'], postHitIpSetToday);
...
function countUniqueVisit(ip, userAgent, countingSet) {
    const hash = generateHash(ip, userAgent);
    if (countingSet.has(hash)) {
        return;
    }

    countingSet.add(hash);
    const statsToday = getStatsGroup(new Date());
    if (countingSet === appHitIpSetToday) {
        statsToday[UNIQUE_APP_HITS_KEY]++;
    } else if (countingSet === postHitIpSetToday) {
        statsToday[UNIQUE_POST_HITS_KEY]++;
    }
}
...
function generateHash(ip, ua) {
    const userAgentToUse = ua || '';
    return crypto.createHash('MD5').update(ip).update('|').update(userAgentToUse).digest('hex');
}
...
// Append a day's stats to the JSONL archive
function archiveDay(date, dayStats) {
    try {
        const record = Object.assign(Object.create(null), { date }, dayStats);
        fs.appendFileSync(visitCountArchiveFilePath, `${JSON.stringify(record)}\n`);
    } catch (err) {
        throw new Error(`Error appending day ${date} to JSONL archive (archiveDay)`, { cause: err });
    }
}
...
function newDayReset() {
    try {
        ...
        postHitIpSetToday.clear();
        searchTermFreqToday.clear();
        printFreqToday.clear();

        // Rotate daily stats: only store the past 7 days, archive the rest
        const statsCutoffDate = new Date();
        statsCutoffDate.setDate(statsCutoffDate.getDate() - 7);
        statsCutoffDate.setHours(0, 0, 0, 0);
        for (const item in visitCounter) {
            ...
            if (savedVisitDate < statsCutoffDate) {
                archiveDay(item, visitCounter[item]);
